...
FAKE_MOCAP = [0,10,20,30,40] # if different from none, it will override the optitrack values
            ...
            while True:
                ...
                if (time.time()-last_loop_time) >= CTRL_LOOP_TIME:
                    ...
                    if sim_flow_tof:
                        ...
                        if board.send_RAW_msg(0x1F01, data=[255,25,0,0,0]):
                            pass
                        else:
                            exit

                        # Fake optical flow [uint8 quality, int32 motionX, int32 motionY]
                        if board.send_RAW_msg(0x1F02, data=[255,0,0,0,0,0,0,0,0]):
                            pass
                        else:
                            exit
