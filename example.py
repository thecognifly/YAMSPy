...
with MSPy(device=serial_port, logfilename='MSPy.log', logfilemode='a', loglevel='DEBUG', use_tcp=True) as board:
    if board==1:
    ...
    for msg in MSPy.MSPCodes.keys():
        if 'SET' not in msg:
            if msg not in avoid_list and not msg.startswith("MSP2_") and not msg.startswith("MSPV2_"):
